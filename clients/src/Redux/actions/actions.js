  ...
  return async (dispatch) => {
    try {
      const paginate = await axios.get(
        `http://localhost:3001/products?page=${page}&size=${size}`
      )
...
export function filterByGenres(filters) {
  return async (dispatch) => {
    ...
    if (filters.valoration.length > 0)
      endpoint += `orderBy=${filters.valoration}&direction=asc&`

    if (filters.price.length > 0)
      endpoint += `orderBy=price&direction=${filters.price}&`
...
export const updateUser = (
  ...
  return async (dispatch) => {
    try {
      ...
      dispatch({ type: UPD_USER, payload: response.data })
    } catch (error) {
...
export const createOrder = (payload) => {
  return async (dispatch) => {
    try {
      const response = await axios.post(
        'http://localhost:3001/mercadopago/create-order',
        payload
      )
